...
### ELB(Elastic Load Balancing) 4가지 종류
| 종류                              | 계층 | 용도 |
|---------------------------------|---|---|
| **ALB**                         | L7 (HTTP/S) | URL·경로·호스트 기반 라우팅 |
| **NLB**                         | L4 (TCP/UDP) | 초고성능, 저지연, 고정 IP |
| **GWLB(Gateway Load Balancer)** | L3 (IP 패킷) | 타사 **보안 어플라이언스** 통합 |
...
| 대상 | 정책(Policy) 직접 부착 | 그룹(Group) 소속 | 역할(Role) 사용 | 비고                              |
| :--- | :---: | :---: | :---: |:--------------------------------|
| **Group** | ⭕ | — | ❌ | 그룹에 부착된 정책은 모든 멤버에게 상속됨         |
| **User** | ⭕ | ⭕ | ❌ | 정책 직접 부착 또는 그룹 상속을 통해 권한 획득     |
| **Role** | ⭕ | ❌ | — | 사용자나 서비스가 Assume(위임)하여 권한을 위임받음 |
| **Service (EC2 등)** | ❌ | ❌ | ⭕ | 반드시 Role(인스턴스 프로파일)을 경유해야 함     |
