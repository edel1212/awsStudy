...
- **하나 이상**의 데이터 센터로 구성
- 각 AZ는 전력, 네트워킹, 그리고 연결성에서 **중복성**을 가지고 있어, 장애가 발생하더라도 **다른 AZ에 영향을 주지 않음**
...
- 네트워크 비용을 절감하고 네트워크를 통과할 수 있는 대역폭을 늘리는 데 도움을 줌
- 쉽게 설명
...
-  `온프레미스 환경`을 AWS 클라우드 스토리지와 통합하기 위한 파일, 볼륨 및 테이프 게이트웨이 인터페이스를 제공

### Amazon FSx

- `Windows 파일 서버`와 호환되는 **완전 관리형 파일 스토리지** 서비스
- 온프레미스 사용자가 하이브리드 스토리지 인프라를 유지하면서 사실상 무제한의 클라우드 스토리지에 액세스 가능
