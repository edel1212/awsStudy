...
- ##### EC2 생성 VPC 연결
  ...
  - DB EC2
    ...
    - `NAT 게이트웨이`를 생성하여 다른 EC2를 통해 외부망의 데이터를 받아 올 수 있다.
      ...
      - `DB-Subnet`이 설정된 라우팅 테이블로 이동 후 라우팅 테이블 편집
      - 외부 접근 IP 대역을 `NAT 게이트웨이`로 연결 후 상위 생성한 NAT 게이트 웨이를 연결 해준다.
        ![alt text](image-25.png)
        - ℹ️ 같은 `VPC`내 지정된 `Subnet의 NAT 게이트웨이`가 아니면 선택 창에 생성이 되지 않는다 (당연한 결과)
