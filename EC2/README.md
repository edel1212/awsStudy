...
- ### 스팟 인스턴스 (Spot Instance)
  - 쉽게 설명하면 주가랑 비슷하다 생각하자 사용하는 사람이 없으면 가격이 싸지고 많아지만 비싸진다.
    - 실시간으로 가격이 유동적으로 바뀐다.
    - 내가 지정한 가격을 넘어서면 인스턴스사 종료되고 그 이하일 경우 인스턴스가 다시 샐행되는 방식
      - 분산 처리가 잘된 환경에서 사용할 경우 굉장한 효율을 낼 수 있다.
- ### 전용 호스트(Dedicated)

  - 실제 물리적인 서버를 임대하는 방식이다.
...
- `AMI`를 사용하여 EC2를 복제하거나 다른 지역(`Reigon`), 계정으로 전달 가능
- 스냅샷을 기반으로 `AMI`를 구성 가능

  - 내가 사용하던 인스턴스를 통해 `AMI`를 생성하면 `EBS` 스냅샷이 추가된 것을 확인 할 수 있다.
    ![Alt text](image-1.png)

- ### 구성

  - 1개 **이상**의 `EBS` 스냅샷
    - 스냅샵을 기반으로 루트 디바이스를 생성함
  - 사용자 권한
...
## Auto Scaling - 오토 스케일링 [ ELB 적용 ]

```properties
# ℹ️ 주의사항 - 삽질 오래함..
#   문제 : Auto - Scaling을 만들 경우 바로 EBL를 만드는 식으로 하면 에러는 없으나 로드 밸런싱에 DNS에 접근이 안되는 문제 발생
#   원인 : `Internet facing(인터넷 경계)` 와 `Internal(내부)` 설정 때문이 였음 ..
#          ㄴ> 👉 로드 밸런서 생성 후에는 스키마를 변경할 수 없습니다.
#   해결 방법 : 내가 사용하려는 경계에 맞게 설정했어야했다.
#              ㄴ> 예제에서는 내부로 하고 인스턴스 접근 서브넷이 외부 였기에 EBL가 정상 작동하지 못했음
```
